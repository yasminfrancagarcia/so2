...
static void so_escalona(so_t *self)
{
  ...
  float melhor_prio_valor = 1e9f; // Um valor inicial muito alto (pior prioridade)

  // Itera pela FILA de prontos (e não pela tabela de processos)
  fila_no *no_atual = self->fila_prontos->inicio;
  while (no_atual != NULL) {
    
    pcb *proc_candidato = achar_processo(self, no_atual->pid);

    // Verifica se o processo é válido e está PRONTO
    if (proc_candidato != NULL && proc_candidato->estado == P_PRONTO) {
      
      // LÓGICA DE PRIORIDADE: menor valor numérico é melhor
      if (proc_candidato->prioridade < melhor_prio_valor) {
        melhor_prio_valor = proc_candidato->prioridade;
        proc_escolhido = proc_candidato; // Achamos um candidato melhor
      }
    }
    no_atual = no_atual->prox;
  }

  // 5. DESPACHA O MELHOR PROCESSO ENCONTRADO
  if (proc_escolhido != NULL) {
    // Encontramos o vencedor. Agora sim removemos ele da fila.
    desenfileira(self->fila_prontos, proc_escolhido->pid);

    // Acha o ÍNDICE do processo escolhido (necessário para self->processo_corrente)
    int indice_escolhido = -1;
    for (int i = 0; i < MAX_PROCESSES; i++) {
      if (self->tabela_de_processos[i] == proc_escolhido) {
        indice_escolhido = i;
        break;
      }
    }
    
    // (Este if deve sempre ser verdadeiro se proc_escolhido não for NULL)
    if (indice_escolhido != -1) {
        // Encontrado! Define como processo corrente.
        self->processo_corrente = indice_escolhido;
        
        console_printf("====> processo %d (prio %.2f) escolhido \n", 
                       proc_escolhido->pid, proc_escolhido->prioridade);
        imprime_fila(self->fila_prontos);

        so_muda_estado(self, proc_escolhido, P_EXECUTANDO);
        
        return; // Termina o escalonamento
    }
  } 

  // Se 'proc_escolhido' for NULL (ex: fila só tinha "lixo" de processos
  // que não estão P_PRONTO) ou se não achou o índice (bug),
  // então não há ninguém para executar.
  self->processo_corrente = NO_PROCESS;
}
...
static void so_chamada_le(so_t *self)
{
  ...
  else
  {
    ...
    atualiza_prioridade(proc); // atualiza a prioridade do processo
